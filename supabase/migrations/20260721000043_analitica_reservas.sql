-- Analítica de reservas dentro del mismo resumen del panel. Se cuenta por
-- la FECHA RESERVADA (no por cuándo se creó), que es lo que le importa al
-- restaurante: cuántas reservas hay cada día, cuántas se cumplen, cuántas
-- no llegan ("no show"), cuánta gente y cuánto se recauda por abonos.
-- Se agrega la clave 'reservations' al JSON; todo lo demás queda igual.
