-- ══════════════════════════════════════════════════════════════════
-- La reserva hecha por la web queda CONFIRMADA de una vez, así el correo que
-- ya salía al crearla (disparador trg_reservation_email) llega diciendo
-- "Reserva confirmada" en lugar de "Reserva recibida".
--
-- Si algún día se activa el abono por persona, la reserva vuelve a quedar
-- pendiente hasta que se pague (no se puede confirmar algo sin abonar).
-- ══════════════════════════════════════════════════════════════════
...
declare
  ...
  v_tbl_loc text;
begin
  begin v_birthday := nullif(p->>'birthday','')::date; exception when others then v_birthday := null; end;
  ...
  if v_client.id is not null then
    -- Cliente que ya existe: no se le exige nada, solo se completa lo que falte.
    ...
    -- Cliente NUEVO: celular y cumpleaños son obligatorios (el correo ya se validó).
    if v_phone_digits is null then
      raise exception 'El celular es obligatorio para reservar';
    end if;
    if v_birthday is null then
      raise exception 'La fecha de cumpleaños es obligatoria para reservar';
    end if;
  ...
  -- Sin abono: queda confirmada al instante. Con abono: pendiente hasta pagar.
  v_status := case when v_deposit > 0 then 'pendiente' else 'confirmada' end;
  ...
  -- El correo lo manda el disparador trg_reservation_email al insertar; como
  -- la reserva ya nace confirmada, sale con el texto "Reserva confirmada".

  return jsonb_build_object(
    'id', v_id, 'code', v_code, 'status', v_status, 'depositRequired', v_deposit
  );
end
$$;
