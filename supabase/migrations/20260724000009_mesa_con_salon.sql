-- ══════════════════════════════════════════════════════════════════
-- En el panel, la mesa asignada ahora viene con el nombre de su salón
-- (ROKA I, ROKA II, TOSCANA, RESTAURANTE, VIP…), no solo el número.
-- ══════════════════════════════════════════════════════════════════
...
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      ...
      'tableZone', (select z.name from restaurant_tables t join zones z on z.id = t.zone_id
                     where t.id = r.table_id),
      'tables', coalesce((
        select jsonb_agg(jsonb_build_object('id', tt.id, 'name', tt.name, 'zone', zz.name)
                         order by tt.sort, tt.name)
        from restaurant_tables tt
        join zones zz on zz.id = tt.zone_id
