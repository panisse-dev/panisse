-- ══════════════════════════════════════════════════════════════════
-- Correo de cancelación: cuando la reserva se cancela (la cancele el
-- cliente desde su enlace o el equipo desde el panel) sale un correo
-- diciendo "Reserva cancelada", con un botón para reservar de nuevo.
-- ══════════════════════════════════════════════════════════════════
...
begin
  ...
  if r.status = 'cancelada' then
    ...
    v_sub := 'tu reserva en PANISSE quedó cancelada. Cuando quieras volver, aquí te esperamos.';
    v_cancelada := true;
  elsif r.status in ('confirmada','cumplida') then
  ...
  v_site_home := coalesce(nullif(btrim(coalesce(v_site,'')),''), 'https://panisse.netlify.app');
  v_manage_url := v_site_home || '/reserva/?id=' || r.id::text;
  ...
  '<div style="background:#eef0ec;padding:24px 12px;font-family:Georgia,Times New Roman,serif;">' ||
    '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;margin:0 auto;background:#f6f6f5;border:1px solid #e3d6b4;">' ||
      '<tr><td style="padding:30px 28px 6px;text-align:center;">' ||
        '<div style="width:56px;height:56px;margin:0 auto;border-radius:28px;background:' ||
          case when v_cancelada then '#8f7c66' else '#11572e' end ||
          ';color:#ffffff;font-size:30px;line-height:56px;">' ||
          case when v_cancelada then '&#10005;' else '&#10003;' end || '</div>' ||
        '<p style="letter-spacing:3px;font-size:11px;color:#8f7434;text-transform:uppercase;margin:16px 0 0;">PANISSE</p>' ||
        '<h1 style="color:' || case when v_cancelada then '#8f7c66' else '#11572e' end ||
          ';font-size:25px;margin:6px 0 4px;">' || v_titulo || '</h1>' ||
      ...
      '<tr><td style="padding:6px 24px;">' ||
        ...
        '</table>' ||
      '</td></tr>' ||
      -- Botones: modificar o cancelar (solo si la reserva sigue viva)
      case when v_cancelada then
        '<tr><td style="padding:20px 28px 26px;text-align:center;">' ||
          '<a href="' || v_site_home || '/reservar/" style="display:inline-block;background:#041b31;color:#d9bb73;text-decoration:none;font-size:14px;font-weight:bold;padding:14px 30px;letter-spacing:0.5px;">Reservar de nuevo</a>' ||
        '</td></tr>'
      else
      ...
      case when v_sede is not null and not v_cancelada then
  ...
  begin
    perform net.http_post(
      ...
      body := jsonb_build_object(
        ...
        'subject', case
          when r.status = 'cancelada' then 'Reserva cancelada · PANISSE'
          when r.status in ('confirmada','cumplida') then 'Reserva confirmada · PANISSE'
          else 'Tu reserva en PANISSE' end,
        'htmlContent', v_html
      )
    );
...
-- Al cancelar (desde la pantalla del cliente) sale el correo de cancelación.
...
-- Y también cuando la cancela el equipo desde el panel.
...
begin
  ...
  -- Al confirmar o al cancelar (y si no lo estaba ya), sale el correo solo.
  if p_status in ('confirmada','cancelada') and coalesce(v_old,'') <> p_status then
