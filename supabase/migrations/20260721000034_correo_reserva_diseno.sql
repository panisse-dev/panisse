-- Rediseña el correo de reserva con la estructura y colores de PANISSE, y
-- agrega el botón "Modificar o cancelar" (abre WhatsApp con el restaurante).
...
begin
  ...
  select name, nullif(regexp_replace(coalesce(whatsapp,''), '\D', '', 'g'), '')
    into v_sede, v_wa from locations where id = new.location_id;
  ...
  v_wa_url := 'https://wa.me/' || v_wa ||
              '?text=Hola%2C%20quiero%20modificar%20o%20cancelar%20mi%20reserva%20%23' || new.code;
  ...
  '<div style="background:#eef0ec;padding:24px 12px;font-family:Georgia,Times New Roman,serif;">' ||
    '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;margin:0 auto;background:#f6f6f5;border:1px solid #e3d6b4;">' ||
      '<tr><td style="padding:30px 28px 6px;text-align:center;">' ||
        ...
        '<h1 style="color:#11572e;font-size:25px;margin:6px 0 4px;">Reserva recibida</h1>' ||
        '<p style="font-size:15px;color:#10202f;margin:6px 0 0;">Hola <b>' || coalesce(new.customer_name,'') ||
          '</b>, tu reserva en PANISSE quedó registrada con éxito.</p>' ||
      '</td></tr>' ||
        ...
        '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #d9bb73;border-bottom:1px solid #d9bb73;">' ||
          '<tr>' ||
            '<td style="padding:13px 4px;text-align:center;border-right:1px solid #eadfbf;">' ||
              '<div style="font-size:17px;color:#041b31;font-weight:bold;">' || new.party_size || '</div>' ||
